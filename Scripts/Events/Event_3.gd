...
@onready var item_type_check: RigidBody3D = %item3_rb
...
#CONTADOR
	if timer_geral.time_left <= event_time and hasPrinted == false: #TROCA DE ESTADO DEPOIS DE CERTO TEMPO
		...
		hasPrinted = true
	if Input.is_action_just_pressed("ui_interact") and timer_geral.time_left <= event_time and isOnArea == true and player.player_has_item == true and event_name == item_type_check.item_type:#AÇÃO DO ITEM
		timer_geral.start()
